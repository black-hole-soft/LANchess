...
namespace Ajedrez
{
    public partial class frmAjedrez : frmConecciones
    {
        public int sec, min, hr;
        public Casilla[,] tablero = new Casilla[8, 8];
        public bool seleccionada, turnoBlanca, amenaza, terminado, tm=true, movida = false;
        public int selectX, selectY, iluminado;
        public Rey reyb = new Rey(4, 7), reyn = new Rey(4, 0);
        HiloComsUDP comsUDP;
        Thread hiloVeriTableroAbierto, hiloMovimiento;
        ...
        public void enroque(int x, int y)
        {
            ...
            movida = false;
            if (x == 1 && y == 7)
                comsUDP.enviaMensaje("partida@MOVIMIENTO@" + comsUDP.nCte1.ToString() + "@" + comsUDP.nOp.ToString() + "@0@7@2@7@R");
            if (x == 6 && y == 7)
                comsUDP.enviaMensaje("partida@MOVIMIENTO@" + comsUDP.nCte1.ToString() + "@" + comsUDP.nOp.ToString() + "@7@7@5@7@R");
            if (x == 1 && y == 0)
                comsUDP.enviaMensaje("partida@MOVIMIENTO@" + comsUDP.nCte1.ToString() + "@" + comsUDP.nOp.ToString() + "@0@0@2@0@R");
            if (x == 6 && y == 0)
                comsUDP.enviaMensaje("partida@MOVIMIENTO@" + comsUDP.nCte1.ToString() + "@" + comsUDP.nOp.ToString() + "@7@0@5@0@R");
            while (!movida)
                Thread.Sleep(10);
            movida = false;
        }
        ...
        {
            hiloVeriTableroAbierto = new Thread(new ThreadStart(hilo_VerificaTableroAbierto));
            hiloVeriTableroAbierto.Start();
            hiloMovimiento = new Thread(new ThreadStart(hilo_Movimiento));
        ...
        private void frmAjedrez_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (comsUDP.nOp != -1)
            {
                comsUDP.enviaMensaje("partida@CERRAR@" + comsUDP.nCte1.ToString() + "@" + comsUDP.nOp.ToString() + "@" + comsUDP.usr + "@" + comsUDP.oponent);
                comsUDP.tablero = false;
            }
            hiloVeriTableroAbierto.Abort();
            hiloMovimiento.Abort();
        }
        private void hilo_VerificaTableroAbierto()
        {
            while (comsUDP.tablero)
            {
                Thread.Sleep(10);
            }
            CierraTablero(this);
        }
        private void hilo_Movimiento()
        {
            while (true)
            {
                while (!comsUDP.movimiento)
                {
                    Thread.Sleep(10);
                }
                accionPieza(comsUDP.x1, comsUDP.y1, "AUTOMATICO");
                accionPieza(comsUDP.x2, comsUDP.y2, "AUTOMATICO");
                movida = true;
                comsUDP.movimiento = false;
            }
        }
